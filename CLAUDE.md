...
- **canonical model** 是解耦「模型来源」与「设备后端」的枢纽契约（承载一等但可选的时间轴 static/time_varying，及模型层级 level）。字段级规范见 `docs/design/00-overall/03c-channel-schema.md`（只存物理量、源/设备无关、层级相关 payload、含阵列几何、可序列化版本化）。
- **模型层级与退化（顶层原则，见 `docs/design/00-overall/03b-model-hierarchy.md`）**：信道表示嵌套 `RT ⊃ GCM ⊃ CDL ⊃ TDL`（TDL 最贴硬件）；用户可在任一层输入，平台沿退化链降到硬件可实现层级，由 RF-SoC 的「参数化 TDL 面」或「CIR 回放面」实现。B 档=退到 TDL+R；A 档=停在 CIR。
