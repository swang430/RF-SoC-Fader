...
**总体设计见 [`docs/design/`](docs/design/README.md)（第一册·顶层设计已冻结 v1.0，tag: design-t1-v1.0；T1-03c 已按升版纪律升至 v1.2——v1.1 完整 PortMap/坐标系声明/簇初相 + v1.2 逐径 DOPPLER（`Ray.doppler_hz?`，随 MPDB 手册 v1.1/HyperRT v3.2.6），2026-07-16。第二册·功能设计已冻结 v1.0，tag: design-t2-v1.0，2026-07-16——11 模块 + T1-15 设计问答/B+ 播放基线。第三册·测试设计已冻结 v1.0，tag: design-t3-v1.0，2026-07-17——T3-00 治理总则～T3-05 CI/发布/验收六篇，测试作为独立监督册约束功能开发，含 HIL 两级环）**。开发执行计划与当前 TODO 台账见 [`docs/plan/DP-01`](docs/plan/DP-01-todo-rt-tdl-first.md)（活文档，2026-07-19 裁定：RT-TDL 生产力工具优先——MS-A 最小闭环（GUI 可控）→ MS-B 输入输出配套 → MS-C 后续台账）。关键约定：
...
- **MIMO 空间相关**：本期交付 B 档（几何相关精确复现 + 衰落统计等效），A 档（时变 CIR 注入）为通用 fader 保留接口——**当前 RF-SoC 不支持 CIR 注入**（2026-07-14 硬件确认），按设备 capabilities 启用，现实出口 .asc 导出（见 `docs/design/00-overall/T1-12-risks-open-items.md`）。**当前硬件不支持 ≠ 平台不设计**：通用能力保留接口、当前设备不实现。
