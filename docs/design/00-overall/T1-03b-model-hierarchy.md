...
| 实现面 | 承载 | 层级要求 | 档 | 状态 |
| :-- | :-- | :-- | :-- | :-- |
| **参数化 TDL 面** | 协议 V3.0 帧：24 抽头 delay/amp/phase/doppler（**帧不含 R 矩阵**；相关性已在 L3 塌缩进逐抽头 amp/phase 确定性权重） | 退化到 **TDL**（R 塌缩入 amp/phase） | B | ✅ 本期 |
| **CIR 回放面** | 时变复抽头（`.asc`/CIR 帧） | 实现到 **CIR** | A | 🔒 **当前 RF-SoC 不支持**（2026-07-14 硬件确认，《12》#1）；作为**通用 fader 实现面保留**，按设备 capabilities 启用（《12》§0）；现实出口 = `.asc` 导出 |

> ⚠️ **R 不进帧**：协议 V3.0 无空间相关字段（《06》），硬件也不"应用 R"。R 由 L3 在退化时**塌缩为逐抽头 amp/phase 确定性权重**（《06》B 方案 Phase 2–4），下方矩阵中的"+R"仅表示"退化过程用到 R"，非"帧携带 R"。
