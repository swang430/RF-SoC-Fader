...
| 15 | 复制帧回传 | u8 | 0 | 0 | 0/1 | ✅ |
| 16 | 路径扩展 | u8 | **0**（io 固定 0x00，docx「多径扩充控制」） | 0 | 0/1；对角信道×8 径，消耗配对信道 | ➕ |
...
| 26 | 信号源频率 | i32 | 0 | 0 | −2³¹ ～ 2³¹−1 × 1/71.58278827Hz | ➕ |
| 27 | 脉冲使能 | u8 | 0 | 0 | 0/1 | ➕ |
...
# 统一策略：编码换算「量化=round、超范围=ValueError」，不静默裁剪（与 §5 一致）。
# 现有 clip_int 系换算（delay/amp/doppler/awgn）在 M1 实现时迁移到同一策略（见 §6）；
# **phase 豁免**：相位保持 2π 卷绕语义（周期量，负值/超 2π 是物理正常输入，非越界），不迁移。
# 调用方（tdl.py）已有显式范围预过滤，迁移不影响现行为。
```
...
| 项 | 动作 |
| :-- | :-- |
| `protocol.py` | ➕ §3.1 七组常量/换算；➕ §3.2 编码器族；ParamID 枚举已全；🔁 现有 clip_int 系换算（delay/amp/doppler/awgn）迁移为「越界 ValueError」；phase 保持 2π 卷绕不迁移（§3.1） |
...
| **流式解析** | 同一帧序列按 1B/7B/整帧/跨帧任意切片喂入 → 产出帧集相同；**分片恰好切在帧头内部**（缓冲尾余 `FD`/`FD B1`）帧不得丢失；帧间插入垃圾字节/截断帧/**超上限长度字段的畸形 0x69 前缀**（不得停摆傻等）→ 重同步且 stats 正确 | **TCP 分片鲁棒性**（M2 依赖的关键性质） |
...
2. ~~ID16/ID21 io 字节取值~~ → 已核对 docx：两者 io 均固定 0x00（ID16 见「多径扩充控制」）；**ID16 payload 仅 0/1 开关，无信道选择字段**——扩展作用于协议固定的对角信道配对方案（docx 列 8 个可扩展信道）。
