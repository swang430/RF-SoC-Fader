...
# 现有 clip_int 系换算（delay/amp/phase/doppler/awgn）在 M1 实现时迁移到同一策略（见 §6），
...
| 项 | 动作 |
| :-- | :-- |
| `protocol.py` | ➕ §3.1 七组常量/换算；➕ §3.2 编码器族；ParamID 枚举已全；🔁 现有 clip_int 系换算迁移为「越界 ValueError」策略（§3.1 统一策略） |
