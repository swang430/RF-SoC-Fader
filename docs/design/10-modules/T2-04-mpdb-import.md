...
- 模式由 **PortMap.link_mode 单一来源**声明（不在 ImportConfig 重复，防双源漂移）+ 对 LINK 表做**分模式核验**（不猜）：
  - **阵元身份解析先行**：MPDB 的 `TX/RX` 可能是收发**设备端点号**而非阵元号，逐天线身份由 `TX/RX_ANT_POSITION` 承载（《MPDB接口的使用》）。核验前先按 `identity_by: "index" | "position"` 解析 LINK↔阵元对应——`position` 模式以坐标对 `arrays.positions_m` 容差匹配（ε 可配），一对一匹配失败即报错。**坐标系框架前提**：MPDB 位置为世界系，而 schema 允许 `positions_m` 为本地系（《T1-03c》§4）——但 schema 现无 `origin`/坐标系声明字段可承载变换参数。**本期规则**：`identity_by="position"` 要求 `positions_m` **直接为世界系**（导入配置显式声明 `frame="world"`，否则拒绝该模式并提示改用 `identity_by="index"`）；本地系+自动变换待 schema 升版（并入 §8-5：AntennaArray 增 `origin`/`frame` 字段）后启用；
...
5. **schema 对齐**（T1-03c 升版项打包）：①AntennaArray.port_map 现为 int[]，不含极化键与 link_mode——完整 PortMap 暂以 provenance 为规范来源，升版时升级为本篇 PortMap 结构；②AntennaArray 缺 `origin`/`frame` 坐标系声明字段——position 身份解析本期仅支持世界系，升版后支持本地系+自动变换；③Cluster 缺 `phase_rad`（簇初相）——GCM/CDL 簇→伪径退化（T2-05 §3）与引擎确定性契约（T2-03 §2）依赖此字段；④复评 RayleighSpec 的 `coeffs_ref` 一等字段——超阈值谱系数的外置已由 model-json codec 序列化层 `$blob` 信封满足（§10 强制性不欠账，T2-10 §4），字段化属可见性增强、非功能必需（T1 已冻结，统一走一次 PR 升版）。
