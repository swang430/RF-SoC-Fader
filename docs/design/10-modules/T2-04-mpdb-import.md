...
| **双源交叉对照** | 同 fixture：上游 DOPPLER 列值 vs 平台 velocity 重算值（双端投影同式） | 逐径数值容差内一致——公式与符号约定的一致性锚 |
