...
```
任一违反 → `ValueError` 带具体冲突项；校验器为纯函数，M2/M5/GUI(M11) 复用。
...
def quantize_delays(delay_s: ndarray) -> tuple[ndarray, QuantReport]
    # round(delay/(1000/120ns))；QuantReport = {clipped_low/high 计数 + 被夹掩码}——
    # 裁剪统计并入 ImportReport（§6），且"被夹到端点"与"真端点码值"可区分，不静默
...
| **量化合并黄金** | 与旧 tdl.py 同输入对照（含相干叠加保相位、0..1050 裁剪计数） | 数值一致（归一化除外——基准外置后按传入 ref 断言） |
