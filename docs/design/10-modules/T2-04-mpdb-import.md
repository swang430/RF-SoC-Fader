...
> 状态：**v1.0 · 已冻结**（2026-07-16，tag: design-t2-v1.0）· §2/§4/§7 已随 MPDB 手册 v1.1（DOPPLER 列）修订（2026-07-16 PR）
...
def load(path: str) -> RawRtTable:
    from HyperRT.MiRT.MPDB import MPDB                        # ★惰性导入，按手册规范路径（函数内 import）
...
- **DOPPLER 可选列（手册 v1.1/HyperRT v3.2.6）**：presence 探测——在场则逐径零换算（Hz）直读入 `Ray.doppler_hz`（v1.2）并记 provenance（`has_doppler` + 源版本若可得）；缺失不报错（旧库零回归，走《T1-05》§5 fallback 链：velocity 重算 → default_doppler_hz）。
- **逐径语义（CIR/CFR）校验**：手册 v1.1 §4.2 披露 CHANNEL 存在 **CFR（合并多径）记录模式**、与逐径 CIR 模式互斥——本 reader 行语义假定**逐径**；可判定为 CFR 的库**显式拒收**（不静默错读；请求过 DOPPLER 的库必为 CIR 可作旁证），判别手段实现期定。
...
- 模式由 **PortMap.link_mode 单一来源**声明（不在 ImportConfig 重复，防双源漂移）+ 对 LINK 表做**分模式核验**（不猜）：
  - **阵元身份解析先行**：MPDB 的 `TX/RX` 可能是收发**设备端点号**而非阵元号，逐天线身份由 `TX/RX_ANT_POSITION` 承载（《MPDB接口的使用》）。核验前先按 `identity_by: "index" | "position"` 解析 LINK↔阵元对应——`position` 模式以坐标对 `arrays.positions_m` 容差匹配（ε 可配），一对一匹配失败即报错。**坐标系框架前提**：MPDB 位置为世界系；schema v1.1 起 AntennaArray 有 `frame`/`origin_m` 坐标系声明（升级项②已落地，《T1-03c》§4）——**本地系+自动变换在 schema 层已可承载**。**本期实现规则不变**：`identity_by="position"` 仍先支持 `frame="world"`（否则拒绝该模式并提示改用 `identity_by="index"`）；本地系自动变换为实现期增强（schema 就绪、实现分期——实现范围不因 schema 升版自动扩大）；
...
V7 键域一致: PortMap 键 ⊆ arrays 阵元域（含 V4 极化展开后的 (element,pol) 域）——
             映射不存在的阵元（如阵列 0..3 却映射 7）→ 报错；
             且被使用阵元（LINK 解析/合成涉及者）必须全部有映射——缺失项列明报错
```
    ...
    model = ChannelModel(
        ...
        provenance=Provenance(source_type="MPDB", source_ref=str(source),
                              import_config={**asdict(cfg),
                                             "portmap": serialize(portmap)}),  # 导入配置全量留痕——
        # ★v1.1 起完整 PortMap 的规范来源是 Meta.port_map（一等字段，上）；此处 provenance 内的
        # 副本仅作导入配置溯源与 v1.0 旧数据兼容读（M5 消费一律读 Meta.port_map）
    )
...
`ImportConfig`（dataclass，沿现有风格）：`fc, topology, path_expansion: bool, identity_by("index"|"position"), frame("world"|"local"，默认 "world"；声明 positions_m 坐标系并原样印到落库 arrays.frame——**分模式值域**：identity_by="position" 仅接受 "world"（传 "local" 拒绝并提示改用 index 模式，§3.1）；identity_by="index" 两者皆可（"local" 仅标注、不做变换，落库为无锚阵列）), position_epsilon_m, max_paths≤24, power_mode(coherent|noncoherent), default_doppler_hz, velocity_tx?/velocity_rx?(平台几何重算 fallback——双端投影需两端速度矢量，各自可选、缺省视为静止 (0,0,0)，与上游生成语义一致), test_mode(conducted|ota)`。**多普勒三级优先**（《T1-05》§5 修订）：上游 DOPPLER 列直读 > `velocity_tx/rx` 平台重算 > `default_doppler_hz`；列与速度矢量同在 → 列优先 + 冗余一致性校验（偏差超容差告警，双源纪律同 portmap）。
...
def merge_bins(delay_code, gain, angles, doppler) -> list[Bin]  # 同 bin 复增益相干叠加（保相位）；noncoherent 仅用于功率统计
                                                            ...
                                                            #   （加权复单位向量求辐角，防 359°/1° 线性平均成 180°），天顶角线性加权
                                                            #   ★bin 内多普勒合并（v1.2）：功率加权线性平均（|gain|² 权重——与手册
                                                            #   POWER_WEIGHTED_MEAN_DOPPLER 同口径）→ 产出 Tap.doppler_hz；整库无
                                                            #   上游列时逐径值先按《T1-05》§5 fallback 链取得（velocity 重算/default）
                                                            #   再同样加权——聚合规则与来源无关，select_strongest/normalize 原样透传
...
- link_mode 与 LINK 表不符、PortMap V1–V7 违例 → `ValueError` 带冲突明细。
...
| **PortMap 校验** | V1–V7 各构造违例(含键域: 幽灵阵元/漏映射) + OTA 双极化展开例 | 全部拒绝且冲突项定位准确 |
| **link_mode/身份解析** | per_element_pair(M×N)与 single_reference(单链路)各一例；数量不符例；identity_by=position 坐标匹配±ε 正负例 | 模式与身份解析核验正确；不猜；link_mode 单源(仅 PortMap) |
...
| **DOPPLER 三分支** | 带列 / 无列 / 列+velocity 双源 各一 fixture | 在场零换算直读+provenance 标记；缺失字段缺省不报错（旧库零回归）；双源列优先+冗余偏差告警 |
| **双源交叉对照** | 同 fixture：上游 DOPPLER 列值 vs 平台双端速度（velocity_tx/rx）重算值（同式） | 逐径数值容差内一致——公式与符号约定的一致性锚 |
| **CFR 拒收** | 可判定为 CFR 模式的库 fixture | 显式拒收带原因，不静默错读 |

---

## 8. 开放问题
1. **MPDB 多快照/时变**：本期 static；多快照序列（A 档时间维、几何多普勒的时间差分）待 RT 侧提供形态后扩展（《T1-05》§5-3）。**轨迹管线设计基线已立**（跨快照路径跟踪/槽位稳定/选径移至跟踪后按后端分档，《T1-15》Q2/D4；B+ 播放《T1-15》§7）。
...
5. ~~**schema 对齐**（T1-03c 升版项打包）~~ → ✅ **已于 v1.1 落地**（2026-07-16 升版 PR，T1-03c §10 迁移规则）：①完整 PortMap 一等化于 `Meta.port_map`（导入直写一等字段，provenance 载体降为 v1.0 兼容读来源）；②AntennaArray 增 `frame`/`origin_m`（本期实现仍先支持 world，§3.1）；③Cluster 增 `phase_rad`；④复评结论=coeffs 外置维持 codec 层 `$blob` 信封、不增字段（T2-10 §4）。

## 9. 本篇验收
- 一份真实 MPDB 可端到端读成 `level=RT` 的合法 schema 实例（零单位换算、天顶角、自包含、可复现）。
- PortMap V1–V7 与两种 link_mode 全覆盖测试通过；M5 可仅凭 §3/§5 接口完成 B 方案编排。
