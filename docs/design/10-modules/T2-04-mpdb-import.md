...
5. **schema 对齐**（T1-03c 升版项打包）：①AntennaArray.port_map 现为 int[]，不含极化键与 link_mode——完整 PortMap 暂以 provenance 为规范来源，升版时升级为本篇 PortMap 结构；②AntennaArray 缺 `origin`/`frame` 坐标系声明字段——position 身份解析本期仅支持世界系，升版后支持本地系+自动变换；③Cluster 缺 `phase_rad`（簇初相）——GCM/CDL 簇→伪径退化（T2-05 §3）与引擎确定性契约（T2-03 §2）依赖此字段（T1 已冻结，统一走一次 PR 升版）。
