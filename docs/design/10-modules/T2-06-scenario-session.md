...
ScenarioSource =(联合类型)
  ...
  | EngineSource(generate_req: GenerateRequest, portmap)              # → M3 generate（GCM/CDL/TDL/CIR；
                                                                      #   seed 在 req 内=确定性契约）
...
- **client_key 不入 Scenario**：EngineSource 提交时由 M6 逐次生成新 UUID（传输幂等键，与 T2-03 §3「provenance 剔除 client_key」同口径）——物理配置与传输键分离，scenario 判等不受提交次数影响。
...
| RESOLVE_FAILED · CLOSED | 终态（CLOSED 释放设备锁） | —— |

- **设备互斥**：`apply` 前取设备锁（per-device asyncio Lock），**ACTIVE 会话持有设备直至 close**；他会话对同一设备 apply → 明确拒（`DeviceBusy`，错误含持有者 session_id），**不做隐式排队**。不同设备并行不受限（《T1-10》单机多会话）。`readback` 只读，不受锁限制（M8 遥测同理）。
- **close 策略**：`close(release="disable" | "leave" | "reset")`，默认 **disable**——经 M2 微帧通道下发「全局使能关」（复用 T2-02 build_control_frame + echo 纪律，同遥测节奏恢复机制），配置保留、可快速重启；`reset`=RESET 清态；`leave`=仅释放锁（交接给外部测量流程时用）。asc 后端 close 无设备语义（直接 CLOSED）。
...
async def resolve(sess) -> ResolvedArtifacts:
    ...
    match scen.source:
        ...
        case EngineSource(req, pm):
            model, rep = await engine.generate(with_fresh_client_key(req), pm)         # M3（异步）
...
async def apply(sess, dry_run=False) -> ApplyResult | Manifest:
    ...
    async with device_lock(sess.device_id):              # 互斥（§3）；DeviceBusy 时立即拒
...
| **close 三策略** | disable / leave / reset | disable 仅发使能关微帧（echo 校验）；leave 零触达；reset 发 RESET |
