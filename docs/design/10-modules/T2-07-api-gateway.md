...
| **审计完备** | control 域全操作 × 成功/失败/被拒；异步含「提交后不轮询」剧本 | 同步=恰一条含 outcome；异步设备触达=网关受理 1 条 + 会话侧 begin/end **成对**（op_id 关联、「每 begin 恰一 end」不变式，无轮询也闭合——T2-10 §3 append-only 语义）；GET 零审计写 |
