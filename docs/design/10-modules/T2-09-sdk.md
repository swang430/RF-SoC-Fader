...
- **`wait()` 语义（终态按操作限定，不设通则）**：轮询 `poll_url`（指数退避，尊重服务端 202 约定）；**以 op_id 关联终局**——202 返回的 op_id 是等待锚点，`op_id ∈ session.completed_ops`（有界历史）才算「本操作」终局、outcome 从对应 OpRecord 取（`current_op == op_id`=仍在途）：re-apply 等复用状态场景，纯看状态会把**旧 ACTIVE** 误判为新操作成功；续等跨越后续操作也能命中历史，被逐出（超 N 条）→ 抛 `StaleWait`（指引查会话审计，T2-06 §2）；
