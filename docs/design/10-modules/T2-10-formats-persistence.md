...
- 原子写分两级：**单文件**=临时文件 + rename（同目录）；**多文件产物**（AscFileSet 等）=**版本目录+符号链接发布，自首写起统一**——写 `<target>.v{N}/`（首写即 v1）→ fsync → `symlink+rename` 原子将链接 `<target>` 指向新版本：`<target>` **永远是符号链接、从不落成实目录**（若首写 rename 成实目录，后续 symlink 替换将无法原子进行）；逐文件提交会留下「部分文件已提交」目录（M2 asc 的 committed 语义是**整集**写盘成功，T2-02 §4），本方案下半写只可能是未被链接引用的 `v{N}` 残留（启动清扫，旧版本进延迟回收、同 §4 GC 宽限）——读方经链接看到的永远是完整集（首写前无链接=明确不存在）。均与 §2 sha256 校验双保险。
