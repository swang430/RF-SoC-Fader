...
> 状态：**v1.0 · 已冻结**（2026-07-16，tag: design-t2-v1.0）· §3②④ 已随《T1-12》N5（输入功率声明制）增声明采集与功率链呈现修订（2026-07-19 PR）
...
| 鉴权 | 短时会话令牌（M7 §4），**HttpOnly Cookie 承载** | 同源部署（静态资源由平台 FastAPI 直服）——REST 与 SSE 同一凭据：**原生 EventSource 无法自设 Authorization 头**，Cookie 随请求自动携带（SSE 鉴权的唯一可行载体）；scope 门控自举：启动时 GET `/auth/me` 取 `{scope, expires, csrf_token}`（**HttpOnly 下 JS 不可解析令牌**，scope 由该端点显式下发，T2-07）；**非安全方法一律携 `X-CSRF-Token`**（环境态 Cookie 的 CSRF 防护——Cookie `SameSite=Strict`+双提交校验，T2-07 §4）——**read 令牌隐藏一切 control 操作入口**（菜单级），服务端仍兜底 403 |
...
**GCM 入口 = 38.901 参数面板**：场景枚举/fc/带宽/**geometry（统计场景必填，表单必填校验与 client 预检同源字段）**/mobility/lsp_mode/**seed 显式可见**（确定性契约的用户认知面）；delay_spread_s 仅 CDL/TDL-x 显示；**tx/rx 阵列 + PortMap 配置区**（GenerateRequest.arrays 无条件必填、EngineSource 携 portmap——复用 RT 向导**步骤 2+3** 的配置与编辑器组件：校验上下文 test_mode/拓扑/path_expansion/目标后端**一并采集**，`/imports/validate` 的 ctx 必需项缺一预检调不了；同源直译）；`want_cir`（A 档时变 CIR）按目标设备 capabilities 灰置（当前 RF-SoC 不支持、asc 出口可用——通用 fader 原则）。→ 组装 EngineSource 存 scenario（提交时 client_key 由服务端注入，UI 不感知）。

**CDL/TDL 入口 = 定表表格编辑器**：3GPP 定表 JSON（`cdl-tdl-table-json/v1`，T2-10）行编辑；**物化参数区**（随提交传入、不入表载荷）：`delay_spread_s`（**必填**——表时延为归一化 delay_norm，×RMS 时延扩展得物理 delay_s，与引擎路径同一缩放语义）、`phase_seed`（TDL 初相种子，默认 0）；**CDL 定表另需 arrays + PortMap 配置区**（复用 RT 向导**步骤 2+3** 组件——含校验上下文采集（ctx 必需项）与 `/imports/validate` 预检——物化时由 reader 写入 `meta.arrays` 与 `Meta.port_map`（schema v1.1 一等字段——M4/M3 的同一落位，T1-03c §4），与 M4/M3 对称：**M5 退化前置校验要求 Meta.port_map 在场、簇伪径导向需要阵列几何**，缺则 resolve 必拒）；**TDL 定表无需 arrays/PortMap**（无阵列语义）**但必须采集栅格拓扑 + 目标信道对**——拓扑（8×8/4×8/2×8/1×8，默认 8×8——`grid.topology` 为 schema 必填、信道对本身推不出拓扑）与 (in,out) 多选（默认 (0,0)，**须 ⊆ 所选拓扑的有效对**——同 V3 相容语义即时校验）：level=TDL **不经 M5**（T2-06 仅对 RT/GCM/CDL 调退化）直通 M2 渲染，渲染按信道对键取 taps——物化时 taps 落 `channels[(in,out)]`、`grid`（topology+channel_pairs）同步（无键则 taps 无处落）→ 提交 POST `/imports`（`kind=cdl_tdl_table`，202，T2-07——定表直录的 API 落点）→ model 句柄 → scenario（ModelRefSource）。**无相位列的提示条分两支**（T2-10 注册表口径）：CDL 定表→经 M5 退化时按 `cluster_phase_seed` 兜底（T2-05 §3 ③）；TDL 定表→直通不经 M5，由 `cdl_tdl_reader` 物化时按**导入请求参数** `phase_seed`（默认 0，随提交传入——不入表载荷，表 schema 冻结零自有字段，T2-10）确定性合成 Tap 初相——两支都把兜底行为告知用户而非静默。

**三入口的共同末步 = 创建 scenario 的「退化参数」面板**：source 物化层级 ∈ {RT, GCM, CDL} 时 **`Scenario.synthesis` 必填**（T2-06 §2——缺则 resolve 期 ScenarioError）——SynthesisConfig 全字段采集：`power_mode`（coherent/noncoherent）、`max_paths`（≤ 目标 capabilities.max_paths，超限即时提示）、`velocity_tx_mps`/`velocity_rx_mps`（可选几何多普勒 fallback 双端速度，各自可选——上游 DOPPLER 列在场时仅作交叉校验，《T1-05》§5 修订）、`rayleigh` 谱配置（可选）、`cluster_phase_seed`（CDL 相位兜底种子，默认 0）；**TDL 直通 / CIR 源不显示该面板**（synthesis 可 None）。→ POST `/scenarios`。
**输入功率声明（场景级、三入口共通，《T1-12》N5）**：可选采集 `InputPowerDecl`（逐输入端口 dBm + 来源标注 user_declared/external_meter，T2-06 §2）——未声明不阻塞创建，但即时提示「功率相关功能将降级：仅相对损耗可用，绝对 P_out/SNR 目标不可用」（T2-08 §3.6 降级面前置告知）。

### ③ 可视化台（UC2/UC3，S2——全部渲染自 canonical model）

**取数面**：→ GET `/models/{id}/view`（**渲染投影** `viz-json/v1`，T2-07——本篇 S1 回路回馈新增：canonical 本体是内部契约不直出 API（《T1-04》§7），服务端投影出渲染数据集，S2 的「渲染自 canonical」经投影兑现——投影只依赖模型，dry-run 无设备仍完整）；CIR 播放等引用型数据 → GET `/blobs/{ref}`（流式）。model 句柄来自 scenario（source/物化结果）或会话（`ResolvedArtifacts.model_id`）。

| 图 | 数据源（canonical 字段——经 `/view` 投影传递，字段名指投影的上游来源） |
| :-- | :-- |
...
| 交互 | → API | 呈现要点 |
| :-- | :-- | :-- |
| 会话创建 | POST `/sessions` | scenario@version × device × backend（asc 可无设备；**rfsoc 可 device=None 建「离线预览会话」**——dry-run/产物下载可用、apply 类恒不在 allowed_ops，T2-06 §2——无真机时帧预览的会话形态；上机=同 scenario@version 新建带设备会话，产物缓存命中） |
...
| 功率链呈现 | GET `/sessions/{id}`（PowerPlan 随 resolve 产物附带，T2-08 §3.6——无新端点） | **声明 P_in → 模型损耗 → 输出衰减（经校准）→ 预计 P_out±不确定度**全链展示，不确定度分解可见（声明精度/定标/插损）；**明示「设备无输入实测，P_out/SNR 基于声明 P_in」**（《T1-12》N5）；本期面板为**只读评估呈现**（目标设定控件——绝对 P_out/SNR——随功率工作流条目与 target API 接线启用，T2-08 §3.6 范围裁定：本期合同无 target 载荷）；未声明 P_in → 评估为 relative 模式 + 场景声明入口引导（T2-08 §3.6 降级面直译） |
...
- 实时曲线：8 输入电平/功率、8 输出含噪/无噪功率与电平；ADC 过载/合路/AWGN 溢出**位图逐位定位端口**；告警条呈现迟滞状态（raised/cleared，不闪烁风暴——服务端已迟滞，前端不再加逻辑）。**★输入侧曲线标注「原码·相对指示」**（《T1-12》N5/T2-08 §2）：轴与图例以原码刻度呈现、不标 dBm，图注明示「设备无输入功率实测——本曲线为未标定相对指示，不可当作 P_in；绝对功率语义见④功率链（基于场景声明）」——防止未标定读数被当实测输入功率回填声明。
...
5. **B+ 播放控制视图（保留，《T1-15》§7）**：随 M7 `/sessions/{id}/playback` 端点与 capabilities 启用（按钮可用性仍由 allowed_ops 驱动，词表届时扩 playback_*）——落地前不出现在任何视图。
