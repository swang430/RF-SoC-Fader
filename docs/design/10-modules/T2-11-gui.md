...
| 功率链呈现 | GET `/sessions/{id}`（PowerPlan 随 resolve 产物附带，T2-08 §3.6——无新端点） | **声明 P_in → 模型损耗 → 输出衰减（经校准）→ 预计 P_out±不确定度**全链展示，不确定度分解可见（声明精度/定标/插损）；**明示「设备无输入实测，P_out/SNR 基于声明 P_in」**（《T1-12》N5）；未声明 P_in → 绝对功率/SNR 控件禁用 + 禁用理由与场景声明入口引导（T2-08 §3.6 降级面直译） |
