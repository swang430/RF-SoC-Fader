...
| 鉴权 | 短时会话令牌（M7 §4） | scope 随令牌下发：**read 令牌隐藏一切 control 操作入口**（菜单级），服务端仍兜底 403 |
...
| 交互 | → API | 呈现要点 |
| :-- | :-- | :-- |
| 列表/检索（名称、标签、层级） | GET `/scenarios` | 卡片含 level 徽标（RT/GCM/CDL/TDL）与最新 version |
| 版本历史 | GET `/scenarios/{id}?version=` | **版本树只读**——「编辑」按钮文案即『另存新版本』（不可变语义前置到 UI 认知） |
...
**CDL/TDL 入口 = 定表表格编辑器**：3GPP 定表 JSON（`cdl-tdl-table-json/v1`，T2-10）行编辑 → 提交 POST `/imports`（`kind=cdl_tdl_table`，202，T2-07——定表直录的 API 落点）→ model 句柄 → scenario（ModelRefSource）。**无相位列的提示条分两支**（T2-10 注册表口径）：CDL 定表→经 M5 退化时按 `cluster_phase_seed` 兜底（T2-05 §3 ③）；TDL 定表→直通不经 M5，由 `cdl_tdl_reader` 物化时按表内可选 `phase_seed`（默认 0）确定性合成 Tap 初相——两支都把兜底行为告知用户而非静默。
...
| resolve / 进度 | POST `/resolve`（202） | RESOLVING 进度 + 失败时 `last_error` problem 字段定位展示 |
| dry-run 预览 | POST `/apply?dry_run=true` | rfsoc=帧摘要表（manifest）；asc=文件预览；产物下载→GET `/artifact`（zip/单信道） |
| apply / 事务进度 | POST `/apply`（202）+ 轮询 | **以 202 返回的 op_id 对齐 `completed_ops`**（同 SDK wait 纪律，防 re-apply 旧态误读）；rolled_back → 展示 OpRecord.result.failure（首个失败帧/原因） |
...
| close | POST `/close` | 三策略选择；DIRTY 态仅 reset 可选（服务端拒的前置呈现） |
...
- 历史回看 → API：以目标 event_id 作 `Last-Event-ID` **另开一路** GET `/telemetry/stream` 读至追平（既有端点承载，无新 API）；该路首事件为 resync（=目标已出缓冲窗）时，连接层**吸收事件但向视图上报「超出缓冲窗」状态**（区别于在线订阅的静默吸收——否则该提示永无触发路径）。
