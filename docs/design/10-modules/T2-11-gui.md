...
| 鉴权 | 短时会话令牌（M7 §4），**HttpOnly Cookie 承载** | 同源部署（静态资源由平台 FastAPI 直服）——REST 与 SSE 同一凭据：**原生 EventSource 无法自设 Authorization 头**，Cookie 随请求自动携带（SSE 鉴权的唯一可行载体）；scope 门控自举：启动时 GET `/auth/me` 取 `{scope, expires}`（**HttpOnly 下 JS 不可解析令牌**，scope 由该端点显式下发，T2-07）——**read 令牌隐藏一切 control 操作入口**（菜单级），服务端仍兜底 403 |
