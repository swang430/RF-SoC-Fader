...
| I-MIG-001 | v1.0 样本迁移链：v1.0 model-json → M10 钩子链 → v1.2（经 v1.1，链式升至最新）→ M5 消费 | 收编 T2-10 §8/§10（黄金矩阵按来源分支）；迁移后模型可被正常退化消费（不止字段对，还能用） |
...
| T2-08 §3/§7 | bypass 未标定 → **直接表访问（`bypass_atten_db`）与显式绝对 target** 抛 `UncalibratedError` 绝不静默按 0（§3.6 现状评估路径**除外**——其降级义务见下行，两行互斥不矛盾）；越域遥测帧丢弃+计数 | U-M8 段 |
