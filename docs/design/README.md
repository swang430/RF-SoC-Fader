...
- **第二册 · 功能设计** `10-modules/`（文件将以 `T2-` 前缀编号，如 `T2-01-protocol-codec.md`）：M1 协议编解码 · M2 设备后端/传输 · M3 引擎集成 · M4 MPDB 导入 · M5 相关性 · M6 场景会话 · M7 API 网关 · M8 遥测校准 · M9 SDK/客户端 · **M10 格式与持久化(File I/O)** · **M11 GUI**（视图/交互/组件级，依《T1-14》）。
